// ambi_split_waveshape_router.js
...
//   fromAmbiAll               : mc.unpack~  (O+1)^2          // unpack all HOA (incl W)
//   toAmbiAll                 : mc.pack~    (O+1)^2          // pack all HOA (incl W)
//   toAmbiWaveshapeminusW     : mc.pack~    (O+1)^2 - 1      // pack non-W to feed waveshaper
//   fromAmbiWaveshapeminusW   : mc.unpack~  (O+1)^2 - 1      // unpack processed non-W back
//
...
function _rebuild_then_wire() {
    ...
    _replace_with_rect("fromAmbiAll",             "mc.unpack~", Math.max(1, total));
    _replace_with_rect("toAmbiAll",               "mc.pack~",   Math.max(1, total));
    _replace_with_rect("toAmbiWaveshapeminusW",   "mc.pack~",   Math.max(1, minusW));
    _replace_with_rect("fromAmbiWaveshapeminusW", "mc.unpack~", Math.max(1, minusW));
...
function _try_wire_now() {
    ...
    var a  = p.getnamed("fromAmbiAll");
    var pa = p.getnamed("toAmbiAll");
    var pm = p.getnamed("toAmbiWaveshapeminusW");
    var fm = p.getnamed("fromAmbiWaveshapeminusW");
    ...
    try { p.message("script", "connect", "fromAmbiAll", 0, "toAmbiAll", 0); } catch(e){}

    // 2) Feed minus-W to waveshaper pack: outlets 1..total-1 -> inlets 0..minusW-1
    for (var k = 0; k < minusW; k++) {
        try { p.message("script", "connect", "fromAmbiAll", k + 1, "toAmbiWaveshapeminusW", k); } catch(e){}
    }

    // 3) Return processed minus-W back into full pack: fm 0..minusW-1 -> pa 1..total-1
    for (var j = 0; j < minusW; j++) {
        try { p.message("script", "connect", "fromAmbiWaveshapeminusW", j, "toAmbiAll", j + 1); } catch(e){}
    }

    // Heuristic “done”: we’ve issued all intended connect commands for this order.
